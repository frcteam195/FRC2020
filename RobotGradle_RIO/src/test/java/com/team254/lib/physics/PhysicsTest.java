...
public class PhysicsTest {
    ...
    public void testDCMotorTransmission() {
        ...
        assertEquals(Units.rpm_to_rads_per_sec(1200.0), motor.free_speed_at_voltage(12.0 + 1.5), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(600.0), motor.free_speed_at_voltage(6.0 + 1.5), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(0.0), motor.free_speed_at_voltage(1.4), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(0.0), motor.free_speed_at_voltage(0.0), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(0.0), motor.free_speed_at_voltage(-1.4), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(-600.0), motor.free_speed_at_voltage(-6.0 - 1.5), kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(-1200.0), motor.free_speed_at_voltage(-12.0 - 1.5), kTestEpsilon);

        assertEquals(.2 * 10.5, motor.getTorqueForVoltage(0.0, 12.0), kTestEpsilon);
        assertEquals(.2 * 3.5, motor.getTorqueForVoltage(0.0, 5.0), kTestEpsilon);
        assertEquals(-.2 * 10.5, motor.getTorqueForVoltage(0.0, -12.0), kTestEpsilon);
        assertEquals(0.0, motor.getTorqueForVoltage(0.0, 0.0), kTestEpsilon);
        assertEquals(0.0, motor.getTorqueForVoltage(0.0, 1.4), kTestEpsilon);
        assertEquals(0.0, motor.getTorqueForVoltage(0.0, -1.4), kTestEpsilon);

        assertEquals(0.0, motor.getTorqueForVoltage(Units.rpm_to_rads_per_sec(1200.0), 13.5), kTestEpsilon);
        assertEquals(-.2 * 1.5, motor.getTorqueForVoltage(Units.rpm_to_rads_per_sec(1200.0), 12.0), kTestEpsilon);
        assertEquals(.2 * 1.5, motor.getTorqueForVoltage(Units.rpm_to_rads_per_sec(1200.0), 15.0), kTestEpsilon);

        assertEquals(0.0, motor.getVoltageForTorque(0.0, 0.0), kTestEpsilon);
        assertEquals(13.5, motor.getVoltageForTorque(Units.rpm_to_rads_per_sec(1200.0), 0.0), kTestEpsilon);
        assertEquals(-13.5, motor.getVoltageForTorque(Units.rpm_to_rads_per_sec(-1200.0), 0.0), kTestEpsilon);
        ...
        for (double speed : speeds) {
            for (double voltage : voltages) {
                ...
                if (Math.abs(voltage) <= 1.5 && Util.epsilonEquals(speed, 0.0)) {
                    assertEquals(0.0, torque, kTestEpsilon);
                } else {
    ...
    public void testDifferentialDrive() {
        ...
        DifferentialDrive drive = new DifferentialDrive(
                70.0,
                84.0,
                Units.inches_to_meters(2.0),
                Units.inches_to_meters(25.5) / 2.0,
                Units.inches_to_meters(25.5) / 2.0 * CalConstants.kTrackScrubFactor,
                transmission,
                transmission);
        ...
        DifferentialDrive.ChassisState velocity = drive.solveForwardKinematics(new DifferentialDrive.WheelState(0.0,
                0.0));
        assertEquals(0.0, velocity.linear, kTestEpsilon);
        assertEquals(0.0, velocity.angular, kTestEpsilon);
        DifferentialDrive.WheelState wheels = drive.solveInverseKinematics(velocity);
        assertEquals(0.0, wheels.left, kTestEpsilon);
        assertEquals(0.0, wheels.right, kTestEpsilon);
        velocity = drive.solveForwardKinematics(new DifferentialDrive.WheelState(Units.rpm_to_rads_per_sec(65.0 * 10.0), Units.rpm_to_rads_per_sec(65.0 * 10.0)));
        assertEquals(11.0, Units.meters_to_feet(velocity.linear), 1.0);
        assertEquals(0.0, velocity.angular, kTestEpsilon);
        wheels = drive.solveInverseKinematics(velocity);
        assertEquals(Units.rpm_to_rads_per_sec(65.0 * 10.0), wheels.left, kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(65.0 * 10.0), wheels.right, kTestEpsilon);
        ...
        wheels = drive.solveInverseKinematics(velocity);
        assertEquals(Units.rpm_to_rads_per_sec(65.0 * 5.0), wheels.left, kTestEpsilon);
        assertEquals(Units.rpm_to_rads_per_sec(-65.0 * 5.0), wheels.right, kTestEpsilon);
        ...
        assertEquals(0.0, dynamics.wheel_torque.left, kTestEpsilon);
        assertEquals(0.0, dynamics.wheel_torque.right, kTestEpsilon);
        assertEquals(0.0, dynamics.wheel_acceleration.left, kTestEpsilon);
        assertEquals(0.0, dynamics.wheel_acceleration.right, kTestEpsilon);
        assertEquals(0.0, dynamics.chassis_acceleration.linear, kTestEpsilon);
        assertEquals(0.0, dynamics.chassis_acceleration.angular, kTestEpsilon);
        dynamics = drive.solveForwardDynamics(
                new DifferentialDrive.ChassisState(0.0, 0.0),
                new DifferentialDrive.WheelState(12.0, 12.0));
        assertEquals(11.0 * .35, dynamics.wheel_torque.left, kTestEpsilon);
        assertEquals(11.0 * .35, dynamics.wheel_torque.right, kTestEpsilon);
        ...
        assertEquals(0.0, dynamics.wheel_torque.left, kTestEpsilon);
        assertEquals(0.0, dynamics.wheel_torque.right, kTestEpsilon);
        assertEquals(0.0, dynamics.voltage.left, kTestEpsilon);
        assertEquals(0.0, dynamics.voltage.right, kTestEpsilon);
        dynamics = drive.solveInverseDynamics(
