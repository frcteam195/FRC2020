...
public class CKAddressableLEDBuffer extends AddressableLEDBuffer {
	...
	@SuppressWarnings("DuplicatedCode")
	public synchronized void stepFadeWithSyncPixel(FloatingPixel f, int pixelRateDivisor, boolean forward) {
		float incDec = 0;
		...
		if (incDec == 0 && mPrevInc > 0 && f.isAtEnd()) {
			f.reset();
			processFloatingPixel(f);
		} else {
